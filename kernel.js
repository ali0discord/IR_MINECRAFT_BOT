...
const reg = require("./Registery.json")
...
bot.loadPlugin(radarPlugin)

bot.once('spawn', () => {
  bot.autoEat.options = {
    priority: 'foodPoints',
    startAt: memory.autoeat_startAt,
    bannedFood: memory.autoeat_bannedFoods
  }
  bot.chat(reg.wel)
  console.log("Sendeing Welcome Messege.")
  // port is the minecraft server port, if first person is false, you get a bird's-eye view
  mineflayerViewer(bot, { port: config.port, firstPerson: true })
  bot.chat("/gamemode survival HeroBrain")
  // Auto Crystal Settings
...
// Radar Options
var options = {
  host: config.radar_options_host,
  port: config.radar_options_port,
}

// Plugin Radar Data Set
...
bot.on('autoeat_started', () => {
  ...
  bot.chat("Man Goshname")
  bot.chat("daram gaza mikhoram")
})

bot.on('autoeat_stopped', () => {
  console.log('Auto Eat stopped!')
  bot.chat("Bah Bah che khoshmaze bood.")
  bot.chat("midoni chi khordam?")
  bot.chat("Pitzza khordam.")
  bot.chat("jat khali.")
  bot.chat("nice bood")
})
...
bot.on('death', function() {
  ...
  bot.chat("I Am death x.x")
  console.log("I Am dead x.x")

})

bot.once('respawn', () => {
  bot.chat("Man Cocha hastam?")
})
...
function followPlayer () {
  const playerCI = bot.players[memory.best_friend]
